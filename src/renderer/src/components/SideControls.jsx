        ...
        {favorites.length > 0 ? (
          <motion.div
            animate={{
              ...
              scale: isExpanded == true ? 1 : 0
            }}
            className={`grid gap-[4px] w-[180px] ${favorites.length == 1 ? 'grid-cols-1' : favorites.length == 2 ? 'grid-cols-2' : 'grid-cols-3'}`}
          >
            {favorites.map((fav, index) => (
              <Favorite key={index} fav={fav} show={isExpanded} />
            ))}
          </motion.div>
        ) : null}
        {/* control buttons */}
